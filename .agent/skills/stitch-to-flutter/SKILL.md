...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `altrupets-monorepo-stitch_to_flutter`                                |
| Version    | 1.0.1                                     |
| Author     | altrupets                                 |
...
**GitHub:** [altrupets/monorepo](https://github.com/altrupets/monorepo) — ⭐  | Forks: 1
...
This Skill provides step-by-step instructions to convert Google Stitch HTML/Tailwind designs into production-ready Flutter/Dart code. It covers parsing Stitch output, mapping Tailwind utility classes to equivalent Flutter widgets and styling (Container, Row/Column, Flex, Text, ThemeData), and generating a maintainable folder structure with Dart files for screens, widgets, assets, and themes. Key features include responsive layout translation (breakpoints to MediaQuery/LayoutBuilder), theme extraction, asset and SVG handling, accessibility attributes, state-management recommendations (Provider/Bloc/Riverpod), routing setup, and performance/animation optimizations. Use cases include migrating web prototypes to mobile apps, speeding cross-platform development, and standardizing UI implementation. Core advantages are reduced manual translation effort, consistent styling and architecture, testability guidance, and practical tips for production readiness (localization, tests, and CI integration).
...
```bash
npx -y @lobehub/market-cli skills install altrupets-monorepo-stitch_to_flutter
```
...
npx -y @lobehub/market-cli skills install altrupets-monorepo-stitch_to_flutter --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install altrupets-monorepo-stitch_to_flutter --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install altrupets-monorepo-stitch_to_flutter --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install altrupets-monorepo-stitch_to_flutter --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install altrupets-monorepo-stitch_to_flutter --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate altrupets-monorepo-stitch_to_flutter --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment altrupets-monorepo-stitch_to_flutter -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment altrupets-monorepo-stitch_to_flutter
```
...
```bash
npx -y @lobehub/market-cli skills comments altrupets-monorepo-stitch_to_flutter
```
